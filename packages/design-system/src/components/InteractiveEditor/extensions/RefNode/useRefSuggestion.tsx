...
/**
 * Discriminated union for suggestion items.
 * Can be either an existing note or a "Create new" placeholder.
 */
export type RefSuggestionItem = MockNote | { createNew: true; title: string };

/** Type guard to check if an item is a "Create new" placeholder */
export function isCreateNewItem(
  item: RefSuggestionItem
): item is { createNew: true; title: string } {
  return 'createNew' in item && item.createNew === true;
}
 ...
 */
export function useRefSuggestion(callbacks?: RefSuggestionCallbacks): UseRefSuggestionReturn {
